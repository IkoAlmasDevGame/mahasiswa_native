...
<body onload="startTime()">
    ...
    $data = mysqli_query($koneksi, "SELECT * FROM tbl_settings WHERE status = '1'");
    $hasil = mysqli_fetch_array($data);
    ?>
    <!-- === Layout Dashboard -->
    <div class="navbar navbar-expand-lg bg-body-secondary position-sticky sticky-sm-bottom">
        <div class="container-fluid">
            <a href="./index.php" class="navbar-brand fs-6 text-start text-dark">Dashboard <?php echo $hasil['nama_universitas'] ?></a>
        ...
        <div class="d-flex justify-content-center align-items-center flex-wrap mt-1 pt-1">
            <div class="card shadow mb-4">
                <div class="card-header py-2">
                    <h4 class="card-title text-center">- Login Administrasi -</h4>
                </div>
                <div class="card-body mt-1">
                    ...
                    $auth = new model\adminAuth($koneksi);
                    $login = new controller\AuthLogin($koneksi);
                    if (!isset($_GET['aksi'])) {
                    } else {
                        switch ($_GET['aksi']) {
                            case 'login-akses':
                    ...
                    ?>
                    <form action="?aksi=login-akses" method="post">
    ...
    <script type="text/javascript">
        function startTime() {
            ...
            document.getElementById('year').innerHTML =
                "&copy <?= $hasil['nama_universitas'] ?> " + tahun + "<br>" + day[today.getDay()] + ", " + h +
                " : " +
                m +
                " : " + s;
